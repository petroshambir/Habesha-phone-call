...
// // 13. Check Device API 👈 (እታ ዝደለኻያ ኣስተኻኽሎ ኣብዚ ኣላ!)
// router.post('/check-device', async (req, res) => {
//     const { phone, deviceId } = req.body;
//     try {
//         const user = await User.findOne({ phoneNumber: phone });

//         // 🛡️ እታ ኣገዳሲት መከላኸሊ መስመር፦ ተጠቃሚ እንተዘይተረኺቡ ንCrash ይከላኸል
//         if (!user) {
//             return res.status(404).json({ success: false, msg: "User not found" });
//         }
...
//             url: 'http://demo.twilio.com/docs/voice.xml', // 👈 ንፈተና ዝኸውን ናይ Twilio ድምጺ (TwiML)
//             to: toNumber, // እቲ ክትፍትን ዝመዝገብካዮ ናይ በሓቂ ቁጽሪ ስልኪ
//             from: process.env.TWILIO_PHONE_NUMBER // እቲ ካብ Twilio ዝዓደግካዮ ቁጽሪ
//         });
...
router.post('/voice', (req, res) => {
    ...
    // answerOnBridge="true" - እዚ እቲ ቀንድ መፍትሒ እዩ
    // timeout="30" - ድሕሪ 30 ሰከንድ ሪንግ እንተ ዘይተለዓለ ጻውዒት ይዓጽዎ
    const twiml = `<?xml version="1.0" encoding="UTF-8"?>
    <Response>
        <Dial callerId="${myTwilioNumber}" timeout="30" answerOnBridge="true">${toNumber}</Dial>
    </Response>`;
