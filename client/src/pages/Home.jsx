...
//     if (!userPhone) return;
//     try {
//       await axios.put(`${BACKEND_URL}/api/auth/update-minutes`, {
//         phone: userPhone,
//         remainingMinutes: formatToDisplay(currentSeconds)
//       });
//     } catch (err) { console.error("❌ DB Update Error", err); }
...
//   // ⏱️ እቲ ናይ ካውንትዳውን ቆጻሪ (In-Call Real-time Countdown)
//   useEffect(() => {
//     let timerInterval;
//     if (isCalling && isAnswered && secondsLeft > 0) {
//       timerInterval = setInterval(() => {
...
//   // 🛑 ዝተስተኻኸለ ናይ ስልኪ ምዕጻው ሎጂክ (ምስ Twilio / Backend ዝተኣሳሰረ)
//   const handleHangUp = async (forcedSeconds = null) => {
//     const finalSecondsToSend = forcedSeconds !== null ? forcedSeconds : secondsLeft;
    
//     // 1. ፈለማ ምስ DB Minutes ኣጣዕም
//     await syncMinutesWithDB(finalSecondsToSend); 

//     // 2. 📞 [ናይ Twilio ምዕጻው] ነቲ ኣብ ሰርቨር ዘሎ ጻውዒት ንምዕጻው ናብ Backend ንልእኽ
...
//   //     // 🔗 ናብ Backend ጻውዒት ይለኣኽ (Twilio Call ንኽጅምር)
//   //     const response = await axios.post(`${BACKEND_URL}/api/call/make-call`, {
...
//   //       // 💡 ቴስት ንምግባር፡ እቲ ስልኪ ምስ ተላዕለ (Answered ምስ ኮነ) ንግዚኡ ባዕሉ Connected ክኸውን እዚ ጌርናዮ ኣለና
//   //       // ኣብ መጻኢ ብ Webhook ክስተኻኸል ይኽእል እዩ።
//   //       setTimeout(() => {
//   //         setIsAnswered(true);
//   //         setCallStatus('connected');
//   //         audioRef.current.pause();
...
//               } else {
//                 if (!number || number.trim() === "") return;
//                 if (number.trim().length < 10) return alert("በጃኻ ቅኑዕ ቁጺሪ የቱ");
//                 startCall();
//               }
//             }}
//           >
...
//   return (
//     <div className="fixed inset-0 w-full flex flex-col items-center select-none overflow-hidden" style={{ background: "#020617", color: "white" }}>
//       <div className="w-full flex justify-between items-start px-6 py-4 shrink-0">
//         <div className="flex flex-col gap-2">
//           <button onClick={onLogout} className="bg-red-600/90 active:scale-95 p-2 px-3 rounded-xl text-[10px] font-black shadow-lg flex items-center gap-1 uppercase tracking-tighter"><LogOut size={14} /> LOGOUT</button>
//           <button onClick={() => navigate("/buy-card")} className="bg-green-600/90 active:scale-95 p-2 px-3 rounded-xl text-[10px] font-black shadow-lg flex items-center gap-1 uppercase tracking-tighter"><CreditCard size={14} /> Buy Card</button>
...
//       <div className="flex-1 w-full flex flex-col justify-between px-6 pb-2 min-h-0 overflow-hidden">
//         {renderTabContent()} 
//       </div>
...
function Home({ phone, onLogout }) {
  ...
  // 🛑 ዝተስተኻኸለ ናይ ስልኪ ምዕጻው ሎጂክ (ምስ Standalone Call Router ዝተኣሳሰረ)
  const handleHangUp = async (forcedSeconds = null) => {
    const finalSecondsToSend = forcedSeconds !== null ? forcedSeconds : secondsLeft;
    ...
    // 2. 📞 [ናይ Twilio ምዕጻው] ነቲ ኣብ ሰርቨር ዘሎ ጻውዒት ንምዕጻው ናብ Standalone /api/call endpoint ንልእኽ
    const currentCallSid = localStorage.getItem("currentCallSid");
    if (currentCallSid) {
      try {
        await axios.post(`${BACKEND_URL}/api/call/hangup-call`, { callSid: currentCallSid });
  ...
  //     // 🔗 ጻውዒት ናብ Standalone Call Router (/api/call) ይለኣኽ
  //     const response = await axios.post(`${BACKEND_URL}/api/call/make-call`, {
  //       fromNumber: userPhone, 
  //       toNumber: targetNumber 
  //     });
  ...
  //       // 💡 ንግዚኡ እቲ ስልኪ ምስ ተላዕለ ባዕሉ Connected ክኸውን እዚ ጌርናዮ ኣለና
  //       setTimeout(() => {
  //         setIsAnswered(true);
  //         setCallStatus('connected');
  //         audioRef.current.pause();
...
const startCall = async (customNumber = null) => {
  ...
  try {
    ...
    console.error("Call initiation failed", error);
    // 404 እንተመጺኡ እዚ ጌጋ እዚ ኢዩ ዝረአ
    alert(error.response?.data?.msg || "ሰርቨር ክራክብ ኣይከኣለን! (404 Error)");
    handleHangUp();
  }
};
